...
define( 'JP_EXPIRES', 1707270414 );

// The following placeholder will be replaced by the Transport Server. If you change / reformat it, make sure to make
// changes in the placeholder replacer code too.
define( 'JP_SECRET', 'RmLXSGhHKq0RQv9spWzIpjniCnKOiTPr' );

// This is used / replaced when the helper script is uploaded over HTTP.
define( 'WP_PATH', '/var/www/html/caprichoslaser.com/' );
...
class DPC_Throw_On_Errors {
	...
	public static function t_fileowner( $filename ) {
		...
		$label = "fileowner( '$filename' )";

		$owner = static::throw_on_warnings(
			function () use ( $filename ) {
	...
	public static function t_filegroup( $filename ) {
		...
		$group = static::throw_on_warnings(
			function () use ( $filename ) {
	...
	public static function t_fseek( $stream, $offset ) {
		...
		$label = "fseek( resource for '" . static::stream_file_path( $stream ) . "', $offset )";

		$fseek_result = static::throw_on_warnings(
			function () use ( $stream, $offset ) {
...
class DPC_Entry_File_Part_Data_Read_State extends DPC_Read_State {
	 ...
	 */
	private static function create_directories_leading_to_file( $path ) {
	 ...
	 */
	private static function filesize_for_file( $path ) {
	 ...
	 *   not met.
	 */
	private static function ensure_file_with_size_and_permissions( $path, $expected_size, $expected_permissions ) {
	...
	/**
	 * Write to a partial file, move it to the final file if we got all the file's data.
	 *
	 * @param resource $chunk_handle      Resource, e.g. an opened file, to read the state's data from.
	 * @param string   $final_file_path   Path where the finished file will end up at, e.g.
	 *                                    "/var/www/html/wp-content/uploads/foo.jpg".
	 * @param string   $partial_file_path Path we will write a partial file, e.g.
	 *                                    "/var/www/html/wp-content/uploads/foo.partial.UNIQUE-FILE-ID.jpg".
	 *
	 * @return DPC_Read_State|null An instance of the next state to run if the current state's data has been
	 *                             read/processed fully, *or* null if the current state ran out of data and needs to be
	 *                             serialized + resumed later.
	 * @throws DPC_Internal_Error_Exception|Exception On various errors and broken expectations.
	 */
	private function write_file_data_and_return_next_state( $chunk_handle, $final_file_path, $partial_file_path ) {

		static::create_directories_leading_to_file( $partial_file_path );

		static::ensure_file_with_size_and_permissions(
			$partial_file_path,
			( 0 === $this->entry_data_read_so_far->file_total_written_so_far_size
				? null
				: $this->entry_data_read_so_far->file_total_written_so_far_size ),
		...
		while ( $this->file_part_written_so_far_size < $this->entry_data_read_so_far->file_part_size ) {
			...
			DPC_Throw_On_Errors::t_fwrite( $partial_file_handle, $file_part_chunk );
		...
		if ( $this->file_part_written_so_far_size === $this->entry_data_read_so_far->file_part_size ) {
			...
			// File part written in full.

			if ( $this->entry_data_read_so_far->file_total_written_so_far_size
			     === $this->entry_data_read_so_far->file_total_size ) {

				// Whole file written in full.
				...
				static::create_directories_leading_to_file( $final_file_path );

				DPC_Throw_On_Errors::t_rename( $partial_file_path, $final_file_path );
	...
	public function process_and_return_next_state( $chunk_handle, $metadata ) {
		...
		if ( ! static::file_path_is_writable( $final_file_path ) ) {
			throw new DPC_Internal_Error_Exception(
				"Final file path '$final_file_path' (or one of its parent directories) is not writable"
			);
		}

		$partial_file_path = static::to_partial_filename(
			$final_file_path,
		...
		try {
			...
			$next_state = $this->write_file_data_and_return_next_state(
				$chunk_handle,
				$final_file_path,
				$partial_file_path
			);
...
function load_db( $error_func = 'fatal_error', $test_connection = false ) {
	...
	if ( $is_dynamic_ssh ) {
		try {
			...
			return array( 'source' => 'environment' );
		} catch ( Exception $exception ) {
	...
	try {
		...
		$wpdb      = new WP_DB_Wrapper( $db_config->user, $db_config->password, $db_config->name, $db_config->host, $db_config->charset, $db_config->collate );
		return array( 'source' => 'parsed wp-config.php' );
	} catch ( Exception $exception ) {
		// 3. finally if all fails, try loading WP with SHORTINIT
		load_wp( false, $error_func, true );
		return array( 'source' => 'WordPress using load_wp fallback' );
	}
}
