		...
		switch ( $handle ) {
			case 'easy-woocommerce-discounts' :
				...
				return array(
					...
					'roles' => $select_data->get_roles(),
					'capabilities' => $select_data->get_capabilities(),
					'countries' => $select_data->get_countries(),
					'states' => $select_data->get_states(),
					'zones' => $select_data->get_shipping_zones(),
					'shippingClasses' => $select_data->get_shipping_classes(),
					'paymentMethods' => $select_data->get_payment_methods(),
					...
					'checkoutFeeList' => $wccs->WCCS_Conditions_Provider->get_checkout_fees(),
					'shippingList' => $wccs->WCCS_Conditions_Provider->get_shippings(),
					'settings' => array(
						'set_min_quantity' => $wccs->settings->get_setting( 'set_min_quantity', 0 ),
						'coupon_url' => home_url( '/' ) . $wccs->settings->get_setting( 'url_coupons_url_prefix', 'coupon' ),
						'url_coupon_redirect_url' => $wccs->settings->get_setting( 'url_coupons_redirect_url', home_url( '/' ) . 'cart/' ),
						'url_coupon_success_message' => $wccs->settings->get_setting( 'url_coupons_success_message', 'Coupon applied successfully' ),
					),
